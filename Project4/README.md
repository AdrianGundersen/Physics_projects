...
| `plotting/2x2.py` + `Ln.cpp`      | `configs/2x2_test.json`    | 2               | Both `use_Trange=true` $\to$ `.json` and `false`$\to$ `.txt`, \(T\in[1,4]\) | 1 x 1                             | 1e6 and 1e7                         | Compare numerics vs analytical solution |
