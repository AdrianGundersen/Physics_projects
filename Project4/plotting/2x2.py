...
# Read out the block for L = 2
L = 2
L2_data = data[str(L)]   # keys in the JSON are strings: "2", "4", ...
...
print(np.average(eps2), np.average(mabs2))
...
plt.figure()
plt.plot(x*part, rel_error_eps, label='relative error')
# plt.axhline(exp_eps_analytical, color='r', linestyle='--', label='Analytical')
...
plt.figure()
plt.plot(x*part, rel_error_mabs, label='relative error')
plt.xlabel(f'Cycles for T = {T}')
...
plt.figure()
plt.plot(x*part, rel_error_cv, label='relative error')
plt.xlabel(f'Cycles for T = {T}')
plt.ylabel(r'$C_V/N$')
plt.grid(True, alpha=0.6)
plt.legend()
...
plt.figure()
plt.plot(x*part, rel_error_chi, label ='relative error')
plt.grid(True, alpha=0.6)
plt.xlabel(f'Cycles for T = {T}')
plt.ylabel(r'$\chi/N$')
plt.legend()
