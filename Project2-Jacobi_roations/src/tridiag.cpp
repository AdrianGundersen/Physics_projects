...
arma::Mat<double> create_tridiagonal(int N, double a, double d){
    ...
    for (int i = 0; i < N; i++){
        for (int j = 0; j < N; j++){
            if (i == j){
                A(i, j) = d; // main diagonal
            }
            else if (i-j == 1){
                A(i, j) = a; // lower diagonal
            }
            else if (j-i == 1){
                A(i, j) = a; // upper diagonal
            }
        }
    }
    return A;
...
double max_offdiag_symmetric(const arma::mat& A, int& k, int& l){
    ...
    for (int j = 1; j < N; j++){ // columns first for cache efficiency
        for (int i = 0; i < j; i++){  // i < j ensures upper triangle
